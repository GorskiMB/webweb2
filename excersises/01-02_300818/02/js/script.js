...
const separeradAlmost = foo => {
  console.log(foo);
  foo.innerHTML = 'Nu är det ändrat!';
};
...
// En funktion med separerad JS-kod och körs direkt
const direct = (() => { // "(function" visar att function "direct()" körs direkt när skriptet körs
  //let btn = document.getElementById('btn');
  let btn = document.querySelector('#btn');
  btn.addEventListener('click', e => {
    console.log('Vad fint du klickade!');
  });
});
...
const changeBackground = () => {
  ...
  bodyTag.style.backgroundColor = e.value;
};
...
const validateForm = e => {
  ...
  console.log(form);
  let firstName = form.firstName.value; // Hämta förnamn
  if (firstName === null || firstName == '') {
    alert('Fyll i ditt förnamn');
    return false; // Hindra formuläret från att köras klart och skickas till php sidan
  }

  if (firstName.length < 2) {
    alert('Du behöver ett namn som är längre');
    return false;
  }
};
